...
// Functions to Create Kitaev Honeycomb Models
...
template<typename T, typename = std::enable_if<std::is_base_of<itensor::SiteSet, T>::value>>
itensor::MPO Create_Kitaev_Honeycomb_Model_2D(int N, int M, std::array<double,3>& K, double J, T& sites, int auxiliaries){
    auto ampo = priv::kitaev_honeycomb_2D(N,M,K,sites,auxiliaries);
    priv::add_heisenberg_interaction(ampo,J,N,M,auxiliaries,"Honeycomb");
    auto H = itensor::toMPO(ampo);
    return H;
}
...
template<typename T, typename = std::enable_if<std::is_base_of<itensor::SiteSet, T>::value>>
itensor::MPO Create_Kitaev_Honeycomb_Model_2D(int N, int M, std::array<double,3>& K, double J, std::array<double,3>& h, T& sites, int auxiliaries){
    auto ampo = priv::kitaev_honeycomb_2D(N,M,K,sites,auxiliaries);
    int length = itensor::length(sites);
    priv::add_heisenberg_interaction(ampo,J,N,M,auxiliaries,"Honeycomb");
    priv::add_magnetic_field(ampo,h,length,auxiliaries);
...
/*
This is the function that will encompass the other functions in later versions
It should be able to take a std::map<std::string, double> that contains all the details for the Hamiltonian

This map will have the following format:
"K": Kitaev Interaction, mandatory
"J": Heisenberg Interaction
"hx", "hy", "hz": Magnetic Interaction
"Gamma": Gamma Interaction
"Gamma_Q": Gamma' Interaction

The Gamma and Gamma' Interactions are not yet implented
*/

template<typename T, typename = std::enable_if<std::is_base_of<itensor::SiteSet, T>::value>>
itensor::MPO Create_Kitaev_Honeycomb_Model_2D(int N, int M, std::map<std::string, double>& H_Details, T& sites, int auxiliaries){
    double k = H_Details["K"];
    std::array<double,3> K = {k,k,k};
    auto ampo = priv::kitaev_honeycomb_2D(N,M,K,sites,auxiliaries);
    int length = itensor::length(sites);
    
    if (H_Details.count("J")){
    ...
    if (static_cast<bool>(H_Details.count("hx") + H_Details.count("hy") + H_Details("hz"))){
        ...
        if (!(static_cast<bool>(H_Details.count("hy")))){
            H_Details["hy"] = 0;
        }
        if (!(static_cast<bool>(H_Details.count("hz")))){
            H_Details["hz"] = 0;
        }
...
// Functions to Create Kitaev Triangular Models
...
// Calculate the Energy over Inverse Temperature with the WI method [DEPRECEATED]
// Use the TDVP implementation below instead
