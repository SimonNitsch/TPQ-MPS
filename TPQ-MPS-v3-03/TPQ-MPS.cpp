...
void Kitaev_Model::Time_Evolution(int TimeSteps, std::vector<double> intervals, int Evols, int init_rand_sites, std::string TDVP_Type, bool Heat_Capacity){
    Calc_Type = false;
...
std::array<std::vector<std::array<double,2>>,2> Kitaev_Model::Calculate_Heat_Capacity(int TimeSteps, std::vector<double>& intervals, std::vector<std::vector<double>>& Energies){
    ...
    En_vec.reserve((TimeSteps*intervals.size()));
    ...
    one_beta2.reserve(TimeSteps);
    std::vector<std::vector<double>> beta1;
    std::vector<double> one_beta1;
    beta1.reserve(intervals.size());
    one_beta1.reserve(TimeSteps);
    ...
    for (int i = 0; i != intervals.size(); i++){
        ...
        for (int j = 0; j != TimeSteps; j++){
            ...
            one_beta1.push_back(curr_beta*(-1));
        }
        beta2.push_back(one_beta2);
        beta1.push_back(one_beta1);
        one_beta2.clear();
        one_beta1.clear();
    }
    ...
    for (int i = 0; i != Energies.size(); i++){
        ...
        for (int j = 0; j != intervals.size(); j++){
            auto first = Energies[i].begin() + j*TimeSteps+1;
            ...
            std::vector<double> tobeint = multiply(diff,beta1[j]);
            if (j != 0){
