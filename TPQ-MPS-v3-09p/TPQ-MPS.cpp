...
/*
std::array<Eigen::MatrixXcd,3> Kitaev_Model::ExpMatrix(int DoubleSpin){
    ...
    for (int i = 0; i != dims-1; i++){
        ...
        double f = std::sqrt(DSdouble*(DSdouble+1.)/4. - sz*(sz+1.));
        Sx(i,i+1) = std::complex<double>(f/2.,0);
        Sx(i+1,i) = std::complex<double>(f/2.,0);
        Sy(i+1,i) = std::complex<double>(0,-f/2.);
        Sy(i,i+1) = std::complex<double>(0,f/2.);
    }
