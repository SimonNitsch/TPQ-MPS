...
void Kitaev_Model::tdvp_loop(std::vector<double>& E_vec, std::vector<double>& C_vec, std::vector<double>& S_vec, std::vector<double>& W_vec, std::array<std::vector<double>,3>& Chi_vec, std::array<std::vector<double>,3>& Mag_vec, itensor::MPS& psi, itensor::Cplx& t, int TimeSteps, itensor::Args& args, itensor::Sweeps& Sweeps, double& cb){
...
void Kitaev_Model::Time_Evolution(int TimeSteps, std::vector<double> intervals, int Evols, int max_sites, int init_rand_sites, std::string TDVP_Type){
    ...
    std::array<std::vector<std::vector<double>>,3> Magnetism;
    for (auto& i : Magnetism){
        i.reserve(Evols);
    }
    ...
    for (int i = 0; i != Evols; i++){
        ...
        for (auto t = T.begin(); t != T.end(); t++){
            tdvp_loop(E_vec,C_vec,S_vec,W_vec,Suscept_vec,Mag_vec,psi,*t,TimeSteps,tdvp_args,Sweeps,curr_beta);
        }
        ...
        for (int i = 0; i != Suscept.size(); i++){
            ...
            Magnetism[i].push_back(Mag_vec[i]);
            Suscept_vec[i].clear();
            Mag_vec[i].clear();
        }
    ...
    Chiy = Mean(Suscept[1]);
    Chiz = Mean(Suscept[2]);
    Magx = Mean(Magnetism[0]);
    Magy = Mean(Magnetism[1]);
    Magz = Mean(Magnetism[2]);
