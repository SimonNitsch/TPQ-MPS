...
class Kitaev_Model{
    ...
    std::vector<std::array<double,2>> Mag;
    ...
    std::array<itensor::MPO,3> magnetization_operators(int LX, int LY, int aux);

    std::vector<std::array<double,2>> Mean(std::vector<std::vector<double>>& M);
    
    void tdvp_loop(std::vector<double>& E_vec, std::vector<double>& C_vec, std::vector<double>& S_vec, std::vector<double>& W_vec, std::array<std::vector<double>,3>& Chi_vec, std::vector<double>& Mag_vec, itensor::MPS& psi, itensor::Cplx& t, int TimeSteps, itensor::Args& args, itensor::Sweeps& Sweeps, double& cb);
