...
void Kitaev_Model::tdvp_loop(std::vector<double>& E_vec, std::vector<double>& C_vec, std::vector<double>& S_vec, std::vector<double>& W_vec, std::array<std::vector<double>,3>& Chi_vec, std::vector<double>& Mag_vec, itensor::MPS& psi, itensor::Cplx& t, int TimeSteps, itensor::Args& args, itensor::Sweeps& Sweeps, double& cb){
    ...
    for (int j = 0; j != TimeSteps; j++){
        ...
        std::cout << E_vec.back() << " " << E << " " << cb << " " << t_beta << "\n" << std::flush;
...
void Kitaev_Model::Time_Evolution(int TimeSteps, std::vector<double> intervals, int Evols, int max_sites, int init_rand_sites, std::string TDVP_Type){
    ...
    std::vector<std::vector<double>> Magnetism;
    std::vector<double> Mag_vec;
    Magnetism.reserve(Evols);
    Mag_vec.reserve(TimeSteps * intervals.size());
