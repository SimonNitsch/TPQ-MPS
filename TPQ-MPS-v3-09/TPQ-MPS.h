...
class Kitaev_Model{
    ...
    itensor::CustomSpinNitsch sites;
    itensor::MPO H_flux;
    ...
    void tdvp_loop(std::vector<double>& E_vec, std::vector<double>& C_vec, std::vector<double>& S_vec, std::vector<double>& W_vec, std::array<std::vector<double>,3>& Chi_vec, itensor::MPS& psi, itensor::Cplx& t, int TimeSteps, itensor::Args& args, itensor::Sweeps& Sweeps, double& cb, double& n);

    void chi_int(itensor::MPS& psi, double n, double t, std::array<std::vector<double>,3>& chi_vec, double step, itensor::Sweeps& sweeps, itensor::Args& args);
    ...
    Kitaev_Model(int LX, int LY, Hamiltonian H_Details, int DoubleSpin, int aux, std::string shape){
        this -> sites = itensor::CustomSpinNitsch((LX*LY+2*aux),{"2S=",DoubleSpin,"ConserveQNs=",false});
